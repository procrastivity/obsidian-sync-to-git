...
Designed, not yet built. Inherits architectural decisions from `05`/`06`/`07`; this doc is the published-images packaging of `07`.
...
│       └── release.yml
├── obsidian-sync/
│   └── Dockerfile          # identical to 06/07
├── reconciler/
│   ├── Dockerfile          # identical to 06/07
│   ├── entrypoint.sh       # identical to 07
│   └── sync-content.sh     # identical to 07
├── README.md
└── LICENSE                 # if publishing publicly
```

The Dockerfiles and scripts are unchanged from `07`. See `07-handoff-vps-content-sync-multi-repo.md`, "The obsidian-sync image" and "The reconciler image" sections, for their contents.
  ...
  build:
    ...
    steps:
      ...
      - name: Parse tag
        id: tag
        run: |
          # Tag format: "obsidian-sync-v1.2.0" → image=obsidian-sync, version=v1.2.0
      ...
      - name: Build and push
        ...
        with:
          context: ./${{ steps.tag.outputs.image }}
          push: true
          tags: |
            ghcr.io/${{ github.repository_owner }}/${{ steps.tag.outputs.image }}:${{ steps.tag.outputs.version }}
            ghcr.io/${{ github.repository_owner }}/${{ steps.tag.outputs.image }}:latest
...
- **`latest` tag floats forward.** Every release also updates `:latest`, so the deployment can pin to `:latest` for ergonomics or pin to a specific version for reproducibility.
...
- **`user:` directive at the service level.** Replaces `07`'s PUID/PGID build args, because pre-built images don't have build args available at deploy time. The runtime `user:` directive sets the UID/GID the container processes run as, which is what matters for the bind-mounted volume permissions.
...
Bring over the Dockerfiles and scripts from `07` (or create them fresh per `06`/`07`'s specs). Add the release workflow at `.github/workflows/release.yml`. Push to GitHub.
...
1. In `content-sync-images`, edit the relevant file (Dockerfile, script).
...
- **Pull-only mode is still load-bearing.** Same reason as `05`/`06`/`07`.
...
1. Create `content-sync-images`, copy in the `obsidian-sync/` and `reconciler/` directories from your `07` project, add the release workflow.
...
4. Edit the compose file: replace each service's `build:` block with `image: ghcr.io/USERNAME/...:${VERSION}`. Replace `args: PUID/PGID` with a top-level `user: "${PUID}:${PGID}"`.
