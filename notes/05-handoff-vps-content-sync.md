...
- **Pull-only sync mode is the *intent*, not currently the *implementation*.** The desired property is that the VPS receives content but never authors it, so accidental mutations don't propagate back to real devices. The bare `obsidian-headless` CLI doesn't expose a pull-only sync mode (despite earlier revisions of this doc claiming it did). In this setup, the practical protection is structural: the reconciliation script only reads from the vault (via rsync as source) and writes to the git working tree — never the reverse. The only writer to the vault directory is `ob sync --continuous`, which faithfully relays remote state. There's no path that mutates the vault locally and feeds it upstream. If you want explicit pull-only enforcement, [Belphemur's containerized variant](https://github.com/Belphemur/obsidian-headless-sync-docker) wraps `ob` with that behavior via `SYNC_MODE=pull-only`.
...
Then disable config category syncing — the server has no reason to send theme, plugin, or workspace state to a machine that's never going to render notes:

```sh
ob sync-config --path /home/obsidian/vault --configs ""
```

Note the flag is `--configs` (plural). An empty value disables config category syncing entirely. The earlier `--mode pull-only` line that lived here in previous revisions of this doc was a fabrication on my part — `ob sync-config` has no such flag. See "Pull-only sync mode" below for what the actual safety story is.
...
#    Only the parts we actually publish. `drafts` is excluded by convention —
#    any directory named drafts in the synced tree stays out of the content
#    repo. See `09-handoff-blog-template.md` for the author workflow.
#    `.github`, `README*`, `LICENSE*` are repo-root infrastructure that lives
#    only in the content repo (not the vault) and must be protected from
#    rsync --delete clobbering them.
rsync -a --delete \
  --exclude='.obsidian' \
  --exclude='.trash' \
  --exclude='.git' \
...
- **The pull-only "protection" is structural, not enforced.** Earlier revisions of this doc claimed `ob sync-config --mode pull-only` was load-bearing for safety. That flag doesn't exist; the bare `obsidian-headless` CLI has no pull-only mode. The actual reason the vault doesn't get polluted by VPS-side mutations is that nothing in this setup writes to it except `ob sync --continuous` itself (which only relays remote state). If a future change adds a process that mutates `/home/obsidian/vault`, that change would propagate to all of your devices — there's no safety net. For explicit enforcement, run Belphemur's containerized variant, which adds pull-only as a wrapper feature.
...
- **Updating `ob`.** When new versions ship, it's `npm install -g obsidian-headless@latest` as the obsidian user, then `sudo systemctl restart obsidian-sync.service`. Check the project README occasionally — the team has been moving fast since launch.
...
- **Where does the obsidian user's `.config/` live across VPS rebuilds?** If the VPS ever needs to be rebuilt (kernel upgrade gone wrong, migration to a new host), reauthenticating `ob login` is fine but worth knowing the auth state isn't in the content repo or anywhere else backed up. Worth a periodic `tar -czf` of `/home/obsidian/.config/obsidian-headless/` to offsite storage just in case. (Note: the directory is `obsidian-headless`, not `obsidian` — `ob` is the CLI from the `obsidian-headless` npm package and uses its own config namespace, separate from the desktop app.)
