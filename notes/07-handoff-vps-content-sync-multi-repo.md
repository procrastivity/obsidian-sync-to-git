...
```
# Host UID/GID used by the runtime `user:` directive in published-image deployments.
# The local-build images currently bake UID/GID 1000 into their image users.
PUID=1000
PGID=1000
...
**Fine-grained PATs are scoped to one resource owner.** If your content repos span multiple GitHub owners (e.g., a personal account and an org you control), one PAT can't cover both — you'll need one PAT per owner.

The reconciler image only knows about `GH_TOKEN`, so the routing happens at the compose layer: each reconciler service gets the appropriate token mapped into its `GH_TOKEN` env. The variable name on the right-hand side is your choice.

Example: content repos under `simensen/...` and `procrastivity/...` need two PATs. The `.env` then has two token variables:

```
SIMENSEN_GH_TOKEN=github_pat_...
PROCRASTIVITY_GH_TOKEN=github_pat_...
```

And the compose file routes the appropriate one per reconciler:
...
Each PAT's repo-list still needs to include every content repo under that owner that the reconciler will push to.

If managing multiple tokens feels like too much overhead, two alternatives:

- **One classic PAT.** Classic PATs aren't tied to a single resource owner — one token can access any repo you have permission to. The trade-off is much broader scope (`repo` covers all repos you can see, not just the ones you select), no per-repo selection, and GitHub steers away from them. Acceptable for personal use; less acceptable for anything shared.
- **Consolidate ownership.** Move all content repos under one owner. Cleanest from a PAT-management standpoint; only worth doing if you have other reasons to consolidate anyway.

The two-PAT pattern is the recommended default for any setup that already has content repos across multiple owners.
...
cd content-sync-vps
cp .env.example .env
$EDITOR .env   # fill in GH_TOKEN and per-site repo URLs
```
...
Disabling config category syncing (the `--configs ""` line; note the plural) keeps theme/plugin/workspace state from being pulled down to a server that won't render notes. Earlier revisions of this doc also included `ob sync-config --mode pull-only` here; that flag doesn't exist on the bare `ob` CLI — see "Things to watch for" for what the actual safety story is. Initial `ob sync` pulls the vault state down; this can take a while depending on vault size.
...
3. Set env vars via Coolify's UI: `GH_TOKEN`, each per-site `*_REPO_URL`, etc. For published-image deployments, set `PUID`/`PGID` only if you are using the runtime `user:` directive; the local-build images themselves bake in UID/GID 1000.
4. Deploy.
...
- **The reconciler may start before the vault is populated.** On a fresh deploy, the `obsidian-sync` container needs anywhere from a few seconds to a few minutes (depending on vault size) to pull the vault contents down. The reconciler's entrypoint and `sync-content.sh` both check for the watch directory's existence and wait quietly if it's missing — one "$WATCH_DIR not yet present; waiting" line at startup, then silence until it appears, then "$WATCH_DIR ready" and normal operation. Without these guards (as in pre-v1.2.0 reconciler images), expect 30–90 seconds of `Couldn't watch / No such file or directory` stderr noise instead. The same guards handle the unlikely case of the directory disappearing mid-operation.
- **Stranded commits get drained on the next sync.** If a `git push` fails (transient network issue, GitHub being slow, PAT expiring mid-sync), the commit stays in the local working tree's `.git` and the next successful sync will push it — even if the new sync produces no working-tree changes itself. The script checks `HEAD` against `origin/$BRANCH` after `git add -A` and pushes when they diverge. Without this check, a commit that fails to push would just sit there until something *else* changed locally.
- **Pull-only sync mode isn't actually configured.** Same correction as in `05` and `06`: bare `obsidian-headless` has no pull-only flag. The `:ro` mount on each reconciler service is the actual mechanism preventing VPS-side mutations from propagating back to your devices. If you want explicit enforcement, swap the `obsidian-sync` service for [Belphemur's image](https://github.com/Belphemur/obsidian-headless-sync-docker), which exposes `SYNC_MODE=pull-only` via env var.
...
- **Should `obsidian-sync` and the reconcilers share an image base?** They don't right now — `obsidian-sync` is `node:24-slim`, reconciler is `alpine:3.23`. Different base images means more disk and slower first build. Not a real cost at any reasonable size; flagged here only because future-you might be tempted to "consolidate" and remember it was a deliberate choice (the reconciler genuinely doesn't need Node).
